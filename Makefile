...
# Generate the DiskQuota-X.X.X.zip package
package:
	@if [ -z "$(VERSION)" ]; then \
	...
	@echo "Creating ZIP archive: DiskQuota-$(VERSION).zip..."
	composer archive --format=zip --file="DiskQuota-$(VERSION)-raw"
	@echo "Repacking into proper structure..."
	mkdir -p tmpzip/DiskQuota && unzip -q DiskQuota-$(VERSION)-raw.zip -d tmpzip/DiskQuota && \
	cd tmpzip && zip -qr ../DiskQuota-$(VERSION).zip DiskQuota && cd .. && rm -rf tmpzip DiskQuota-$(VERSION)-raw.zip
