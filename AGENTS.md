...
Supported PHP: `>=8.1` (`composer.json`); check `config/module.ini` for Omeka compatibility.
...
Read [quota-enforcement](.agents/skills/quota-enforcement/SKILL.md) when you need to: change upload quota checks, user/site quota settings, or disk-usage queries.
